...
class EmpleadoDao:
    ...
    def buscarEmpleado(self, rut_trabajador):
        ...
        FROM empleados e
        LEFT JOIN datos_laborales dl ON e.ID_EMPLEADOS = dl.ID_EMPLEADOS
        LEFT JOIN contacto_emergencia ce ON e.ID_EMPLEADOS = ce.ID_EMPLEADOS
        LEFT JOIN carga_familiar cf ON e.ID_EMPLEADOS = cf.ID_EMPLEADOS
        WHERE e.RUT = %s
        """, valor)
    ...
    def eliminarEmpleado(self, rut_trabajador):
        ...
        if empleado is not None:
            try:
                ...
                # Eliminar de las tablas dependientes
                ...
                self.mysql.cursor.execute("DELETE FROM contacto_emergencia WHERE ID_EMPLEADOS = %s", (empleado_id,))
                self.mysql.cursor.execute("DELETE FROM datos_laborales WHERE ID_EMPLEADOS = %s", (empleado_id,))
                self.mysql.cursor.execute("DELETE FROM carga_familiar WHERE ID_EMPLEADOS = %s", (empleado_id,))

                # Eliminar de la tabla empleados
                self.mysql.cursor.execute("DELETE FROM empleados WHERE RUT = %s", (rut_trabajador,))
    ...
    def actualizarEmpleado(self, empleado_actualizado):
        ...
        if empleado_existente is not None:
            ...
            try:
                ...
                query_datos_laborales = "UPDATE datos_laborales SET CARGO = %s, FECHA_INGRESO = %s, AREA = %s, DEPARTAMENTO = %s WHERE ID_EMPLEADOS = (SELECT ID_EMPLEADOS FROM empleados WHERE RUT = %s)"
                ...
                query_contacto_emergencia = "UPDATE contacto_emergencia SET NOMBRE_CONTACTO = %s, RELACION = %s, TELEFONO_CONTACTO = %s WHERE ID_EMPLEADOS = (SELECT ID_EMPLEADOS FROM empleados WHERE RUT = %s)"
    ...
    def obtenerEmpleados(self):
        try:
            self.mysql.cursor.execute("SELECT ID_EMPLEADOS, RUT, NOMBRE, GENERO_EMPLEADO, DIRECCION, TELEFONO FROM EMPLEADOS")
    ...
    def empleadosPorCargo(self, cargo):
        try:
            query = """
                SELECT e.RUT, e.NOMBRE, e.GENERO_EMPLEADO, e.DIRECCION, e.TELEFONO 
                FROM EMPLEADOS e
                JOIN DATOS_LABORALES dl ON e.ID_EMPLEADOS = dl.ID_EMPLEADOS
                WHERE dl.CARGO = %s
            """
